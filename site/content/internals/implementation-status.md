...
| evaluator and structured values | Implemented | evaluator tests and 1,357 corpus cases | dynamic/opaque values retain tag-level rather than static element schemas |
...
| prompt | Implemented, host-limited | pure renderer snapshots and authenticated host producers | optional battery/custom segments depend on host capability and configuration |
...
| `shoal-leash` | effects, plans, policy, enforcement reporting/backends | Implemented, platform-limited | policy unit tests, backend integration, and live TCP denial | coarse denial needs Landlock ABI 4+ or Seatbelt; destination allowlists unavailable |
...
| `shoal-config` | typed config discovery, merge, env overrides, validation | Implemented | loader/validation tests | rich prompt and Reef retain subsystem-owned schemas |
| `shoal-prompt` | pure prompt model, style, module rendering | Implemented | snapshots and renderer benchmark | optional producer data is host-owned and cached outside rendering |
...
| interpreter blocks | Implemented | dynamic parser-context and real adapter-lowering tests | shipped default names have generated parity; custom names require the host parse context |
...
**1,357 cases across 79 suites**. An exact-toolchain run on 2026-07-18 observed **1,353 passed, 0
...
|---|---:|---:|---:|---|
| null/bool/int/float | yes | yes | inline | bool `.str`/`.display` are executable and receiver-advertised |
...
| list/record/table/range | yes | yes | recursive with budget | list/table/range integer `.get` and record string `.get` are executable and advertised |
...
The previously documented receiver drift is closed: table/range `.get` dispatch and boolean
`.str`/`.display` metadata agree with runtime behavior and have owning behavioral fixtures. The
registry supplies completion and did-you-mean candidates; dispatch remains authoritative for exact
arity and argument-value validation. See [value method dispatch](../value-method-dispatch/).
...
| RPC attachment admission | Implemented centralized classification | every routed stateful method rejects before parameter decode; only attach/parse/complete are pre-attachment surfaces |
...
Core config and rich prompt loading share one nearest-ancestor project-file discovery rule. Rich
prompt details remain subsystem-owned so core validation treats `[prompt]` as an opaque table while
the bounded prompt parser validates its exact schema and migrates legacy `prompt.template`.
...
- Git counts, including stash count, come from one bounded status probe;
...
Interpreter-class adapters feed an explicit parser-context set and own raw-block argv/payload/output
lowering. A generated bundled-pack parity test prevents shipped interpreter declarations from
drifting outside the standalone parser's default registry.
