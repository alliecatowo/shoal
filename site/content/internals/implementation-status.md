...
group = "Maintenance"
eyebrow = "Reality map"
status = "Reconciled against fcb10bd: 2026-07-17"
...
| evaluator and structured values | Implemented | evaluator tests and 1,331 corpus cases | function type checking and method metadata have holes |
| builtins and namespaces | Implemented | canonical command precedence plus corpus | method metadata remains a separate incomplete authority |
...
| `shoal-eval` | scopes, calls, commands, builtins, effects, modules, Reef | Implemented | largest corpus and integration surface | child creation and command-source precedence are unified; method metadata is not |
...
| `shoal-auth` | token persistence and validation | Implemented, host-limited | multiprocess store tests and per-request kernel revalidation | bearer profile/caps are machine authority metadata; no live token-admin RPC |
...
**1,331 cases across 78 suites**. The last explicitly recorded full audit run on 2026-07-16 was
**1,306 passed, 0 failed, 4 skipped** from the then-1,310-case corpus; do not present that historical
run as evidence for the 20 newer cases. Skips are host-dependent and are not proof of skipped behavior.
...
| R4 ports/modularization | Structural split, effect ports, and canonical command-source precedence are implemented; some direct host calls and method metadata debt remain. |
...
5. method metadata still duplicates part of runtime dispatch truth;
6. filesystem/network sandbox coverage is platform- and effect-limited;
7. plugin ABI v1 is deliberately narrow and synchronous component compilation is only byte-bounded;
8. auth has no live token-administration RPC even though file changes are observed immediately;
9. global quotas bound retained kernel resources, but in-language worker fan-out is outside them;
10. Windows remains a separate architecture project.
