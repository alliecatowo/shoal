...
| `shoal-auth` | token persistence and validation | Implemented, host-limited | multiprocess store tests and per-request kernel revalidation | bearer profile/caps are descriptive metadata, not grants; no live token-admin RPC |
...
**1,331 cases across 78 suites**. An exact-toolchain run on 2026-07-17 observed **1,327 passed, 0
...
| `journal.enabled/state_dir` | yes | REPL/private kernel; durable kernel uses its CLI state root | implemented, host-specific |
...
9. kernel quotas and separate evaluator budgets bound retained sessions, 64 native workers per
