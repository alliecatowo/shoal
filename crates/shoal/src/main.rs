...
fn run_source(
    ...
    apply_render_color_config(bootstrap.config().render.color);
    ...
    let echo_mode =
        shoal_host::resolve_echo_mode(bootstrap.config().render.echo.as_deref(), EchoMode::Quiet);
    let configured_width = bootstrap.config().render.width;
