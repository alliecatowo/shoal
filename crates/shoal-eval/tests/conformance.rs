...
fn normative_conformance_corpus() {
    let mut files: Vec<_> = fs::read_dir(cases_dir()).unwrap().map(|e| e.unwrap().path())
        .filter(|p| p.extension().is_some_and(|x| x == "toml")).collect();
    files.sort();
    assert!(!files.is_empty(), "no conformance suites found");
    let mut ran = 0usize;
    for file in files {
        let text = fs::read_to_string(&file).unwrap();
        let suite: Suite = toml::from_str(&text).unwrap_or_else(|e| panic!("{}: {e}", file.display()));
        for case in suite.case {
            if case.skip.is_some() { continue; }
            ran += 1;
            run_case(&file, &case);
        }
    }
    assert!(ran >= 75, "normative corpus must exercise at least 75 non-skipped cases; ran {ran}");
}
...
fn run_case(file: &Path, case: &Case) {
    ...
    for fixture in case.fixture.as_deref().unwrap_or_default() {
        ...
        if fixture.ends_with('/') { fs::create_dir_all(&path).unwrap(); }
        else { fs::create_dir_all(path.parent().unwrap()).unwrap(); fs::write(path, []).unwrap(); }
    }
    ...
    if case.parse_error.unwrap_or(false) {
        ...
        if let Some(needle) = &case.parse_error_contains { assert!(err.to_string().contains(needle), "{label}: {err}"); }
        return;
    }
    ...
    if let Some(code) = &case.error {
        ...
        if let Some(needle) = &case.error_contains { assert!(err.msg.contains(needle), "{label}: {err}"); }
    } else {
        let value = result.unwrap_or_else(|e| panic!("{label}: unexpected eval error: {e}"));
        assert_eq!(render_inline(&value), case.value.as_deref().expect("case needs value/error/parse_error"), "{label}");
    }
}
