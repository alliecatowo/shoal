...
fn on_handler_inherits_the_spawn_gate() {
    ...
    let mut ev = restricted_evaluator(d.path());
    let src = format!(
        "let results = channel(\"res\").events()\n\
         let listener = on(channel(\"cmd\"), (ev) => {{ channel(\"res\").emit(try {{ ({}).ok }} catch {{ \"DENIED\" }}) }})\n\
         channel(\"cmd\").emit(1)\n\
         let result = results.take(1).take_until(every(30s)).map(ev => ev.payload).collect().first()\n\
         listener.cancel()\n\
         result",
        cat_src(&ok)
    );
...
fn on_handler_inherits_config() {
    let (d, _ok) = scene();
    let mut ev = config_evaluator(d.path());
    let src = "let results = channel(\"cfg\").events()\n\
               let listener = on(channel(\"cmd\"), (ev) => { channel(\"cfg\").emit(config.get(\"b_marker\")) })\n\
               channel(\"cmd\").emit(1)\n\
               let result = results.take(1).take_until(every(30s)).map(ev => ev.payload).collect().first()\n\
               listener.cancel()\n\
               result";
...
fn on_handler_inherits_reef_resolution() {
    ...
    let src = "let results = channel(\"res\").events()\n\
               let listener = on(channel(\"cmd\"), (ev) => { channel(\"res\").emit(try { (faketool).ok } catch { \"MISS\" }) })\n\
               channel(\"cmd\").emit(1)\n\
               let result = results.take(1).take_until(every(30s)).map(ev => ev.payload).collect().first()\n\
               listener.cancel()\n\
               result";
