...
impl Evaluator {
    ...
    /// it only ever ensures the cached [`ScopeChain`] (a pure filesystem walk,
    /// at most once per command, and a no-op at all when the cwd hasn't
    /// changed) and reads the already-loaded [`shoal_reef::Lockfile`]. It never
    /// calls into [`Resolver::resolve`], so it never probes a `--version`,
