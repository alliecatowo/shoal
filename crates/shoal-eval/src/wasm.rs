...
impl CapabilityProvider for SessionCapabilities {
    ...
    fn read_file(&self, path: &Path) -> Result<Vec<u8>, CapabilityError> {
        ...
        self.fs.read(&path).map_err(|error| CapabilityError {
        ...
        for argument in &call.args {
            match argument {
                CmdArg::FlagLong { name, value, .. } => {
                    args.push(PluginValue::Text(format!("--{name}")));
                    if let Some(value) = value {
                        for value in self.expand_arg(value)? {
                            args.push(into_plugin_value(value)?);
                        }
                    }
                }
                CmdArg::FlagShort { chars, .. } => {
                    args.push(PluginValue::Text(format!("-{chars}")));
                }
                CmdArg::DashDash { .. } => args.push(PluginValue::Text("--".into())),
                _ => {
