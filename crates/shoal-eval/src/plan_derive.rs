...
impl Evaluator {
            ...
            Expr::FnCall { name, args, span } => {
                ...
                match name.as_str() {
                    ...
                    other => {
                        ...
                        } else if self.exec.env.get(other).is_some_and(|v| v.is_callable()) {
    ...
    pub(crate) fn plan_call(
        ...
        if !call.forced
            ...
            && self.exec.env.get(&call.head).is_some()
        {
            return Ok(());
        }
        ...
        if let Some(adapter) = self.host.adapters.lookup(&call.head).cloned() {
            ...
            for declared in &spec.effects {
                for effect in parse_declared_effect(declared, &bindings, &self.exec.cwd) {
                    push_effect(out, effect);
                }
            }
