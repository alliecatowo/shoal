...
impl Evaluator {
    pub(crate) fn bind_pattern(&mut self, p: &Pattern, v: Value, mutable: bool) -> VResult<()> {
        match p {
            Pattern::Wildcard { .. } => Ok(()),
            Pattern::Bind { name, .. } => {
                self.exec.env.declare(name.clone(), v, mutable);
                Ok(())
            }
            Pattern::Lit { expr, .. } => {
    ...
    pub(crate) fn pattern_matches(&mut self, p: &Pattern, v: &Value) -> VResult<bool> {
        match p {
            ...
            Pattern::Bind { name, .. } => {
                self.exec.env.declare(name.clone(), v.clone(), false);
                Ok(true)
            }
            ...
            Pattern::Type { ty, name, .. } => {
                if v.type_name() == ty.name {
                    if let Some(n) = name {
                        self.exec.env.declare(n.clone(), v.clone(), false);
                    }
                    Ok(true)
                } else {
                    Ok(false)
            ...
            Pattern::Record { fields, .. } => {
                ...
                for f in fields {
                    ...
                    match &f.pattern {
                        ...
                        None => self.exec.env.declare(f.name.clone(), fv, false),
                    }
                }
                Ok(true)
            }
