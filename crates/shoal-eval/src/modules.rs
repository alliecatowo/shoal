...
impl Evaluator {
    ...
    pub(crate) fn eval_use(&mut self, path: &str, span: Span) -> VResult<()> {
        ...
        self.exec.env.declare(stem, exports, false);
        Ok(())
    }
    ...
    fn load_module(&mut self, canon: &Path) -> VResult<Value> {
        if let Some(cached) = self.exec.modules.get(canon) {
            return Ok(cached.clone());
        }
        if self.exec.module_stack.iter().any(|p| p == canon) {
        ...
        let saved_env = std::mem::replace(&mut self.exec.env, Env::root());
        let module_dir = canon
            .parent()
            .map(Path::to_path_buf)
            .unwrap_or_else(|| self.exec.cwd.clone());
        let saved_cwd = std::mem::replace(&mut self.exec.cwd, module_dir);
        // A module's top-level decls are not inside a fn body; reset the guard so
        // module setup can run, then restore.
        let saved_in_fn = std::mem::replace(&mut self.exec.in_fn_body, 0);
        self.exec.module_stack.push(canon.to_path_buf());
        ...
        self.exec.module_stack.pop();
        self.exec.env = saved_env;
        self.exec.cwd = saved_cwd;
        self.exec.in_fn_body = saved_in_fn;
    ...
    /// module env into a record. Reads from `self.exec.env`, which is still the module
        ...
        for stmt in &program.stmts {
            match stmt {
                Stmt::Fn { decl } if decl.exported => {
                    if let Some(v) = self.exec.env.get(&decl.name) {
