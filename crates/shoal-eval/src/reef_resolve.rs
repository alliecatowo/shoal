...
impl Evaluator {
    ...
    pub(crate) fn ensure_reef_chain(&mut self) {
        let fresh = match &self.exec.reef.chain {
            Some((cwd, _)) => cwd != &self.exec.cwd,
            None => true,
        };
        if !fresh {
            return;
        }
        let chain = ScopeChain::discover(&self.exec.cwd, self.host.reef_user_manifest.as_deref());
        ...
        self.exec.reef.lock = self
            ...
            .unwrap_or_default();
        self.exec.reef.chain = Some((self.exec.cwd.clone(), chain));
    }
