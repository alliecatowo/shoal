...
- Path accessors are field-reachable as well as method-reachable: the zero-arg `path` accessors `.name .stem .ext .parent` (pure) and `.exists .is_dir .is_file .size .modified` (fs-backed, via the `Fs` port) resolve through plain field access, so the `.field` shorthand in implicit lambdas reaches them — `glob("*.rs").map(.name)`, `ls.where(.size > 1mb)`. Argument-taking path methods (`.join .abs .read .save .append`) stay method-only. (`crates/shoal-eval/src/expr_access.rs::field`.)
